 ...
 */
export function formatEmailMarkdown(subject, author, to, emailThread) {
 ...
 */
export function formatForDisplay(state, toolCall) {
    ...
    for (const message of messages) {
        ...
        if (typeof message === 'object' && message !== null) {
            ...
            if (message.tool_calls && Array.isArray(message.tool_calls)) {
                toolCallNames.push(...message.tool_calls.map((call) => call.name.toLowerCase()));
            }
            // Handle class instances with toolCalls property
            else if ('toolCalls' in message && Array.isArray(message.toolCalls)) {
                toolCallNames.push(...message.toolCalls.map((call) => call.name.toLowerCase()));
            }
        }
    }
 ...
 */
export function formatEmailOptional(subject, author, to, emailThread) {
    return formatEmailMarkdown(subject ?? "No Subject", author ?? "Unknown Sender", to ?? "Unknown Recipient", emailThread ?? "");
}
