...
import { AGENT_MODULE, setAgentModule, setupAssistant, testEmails, runInitialStream, runStreamWithCommand } from './utils/test-utils.js';
...
describe('HITL functionality tests', () => {
    ...
    test('Accept write_email and schedule_meeting flow', async () => {
        ...
        const finalChunks = await runStreamWithCommand(emailAssistant, new Command({ resume: [{ type: "accept" }] }), threadConfig);
        // Verify completion with Done tool call
        const doneMessage = finalChunks.find(chunk => {
            if (chunk.ai_message) {
                const message = chunk.ai_message;
                return message.tool_calls && message.tool_calls.some((tc) => tc.name === 'Done');
            }
            return false;
        });
    ...
    test('Edit tool call parameters', async () => {
        ...
        const secondChunks = await runStreamWithCommand(emailAssistant, new Command({ resume: [{ type: "edit", args: editedArgs }] }), threadConfig);
    ...
    test('Reject tool call with feedback', async () => {
        ...
        const secondChunks = await runStreamWithCommand(emailAssistant, new Command({ resume: [{ type: "reject", args: "I'm not available next week. Please suggest the following week instead." }] }), threadConfig);
