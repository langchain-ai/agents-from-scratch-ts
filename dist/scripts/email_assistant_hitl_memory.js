...
// LangGraph imports
import { StateGraph, START, END, Command, MemorySaver, InMemoryStore } from "@langchain/langgraph";
import "@langchain/langgraph/zod";
// LOCAL IMPORTS
import { getTools, getToolsByName } from "../lib/tools/base.js";
import { HITL_MEMORY_TOOLS_PROMPT, triageSystemPrompt, triageUserPrompt, agentSystemPromptHitlMemory, defaultBackground, defaultResponsePreferences, defaultCalPreferences, defaultTriageInstructions } from "../lib/prompts.js";
import { EmailAgentHITLState } from "../lib/schemas.js";
import { parseEmail, formatEmailMarkdown, formatForDisplay } from "../lib/utils.js";
 ...
 */
async function getMemory(store, namespace, defaultContent = "") {
    try {
...
async function updateMemory(store, namespace, messages) {
    try {
        ...
        const userPreferences = await store.get(namespace, "user_preferences");
        const currentProfile = userPreferences ? userPreferences.value : "";
        ...
        const systemMsg = {
            type: "system",
            content: MEMORY_UPDATE_INSTRUCTIONS.replace("{current_profile}", currentProfile).replace("{namespace}", namespace.join("/"))
        };
        ...
        const messagesForLLM = formattedMessages.map(msg => {
            if (msg.role === "user") {
                ...
                return { type: "ai", content: msg.content, tool_calls: [] };
            }
            else {
 ...
 */
export const createTriageRouterNode = (llm, store) => {
    return async (state) => {
        try {
            ...
            const triagePreferences = await getMemory(store, ["email_assistant", "triage_preferences"], defaultTriageInstructions);
 ...
 */
export const createTriageInterruptHandlerNode = (store) => {
    return async (state) => {
        ...
        try {
            // Create messages
            const messages = [
                { type: "human", content: `Email to notify user about: ${emailMarkdown}` }
            ];
            ...
            if (reviewAction === "continue" || reviewAction === "feedback") {
                ...
                if (reviewData) {
                    returnMessages.push({ type: "human", content: `User wants to reply to the email. Use this feedback to respond: ${reviewData}` });
                }
                // Update memory with feedback
                const memoryUpdateMessages = [
                    { type: "human", content: "The user decided to respond to the email, so update the triage preferences to capture this." },
                    ...returnMessages
                ];
                await updateMemory(store, ["email_assistant", "triage_preferences"], memoryUpdateMessages);
                goto = "response_agent";
            }
            else {
                // User ignored the email or other action
                const memoryUpdateMessages = [
                    { type: "human", content: "The user decided to ignore the email even though it was classified as notify. Update triage preferences to capture this." },
                    ...returnMessages
                ];
                // Update memory with feedback
                await updateMemory(store, ["email_assistant", "triage_preferences"], memoryUpdateMessages);
            ...
            return new Command({
                ...
                update: {
                    messages: [
                        { type: "system", content: `Error in triage interrupt: ${error.message}` }
                    ]
                }
            });
        }
 ...
 */
export const createLLMCallNode = (llmWithTools, store) => {
    return async (state) => {
        try {
            // Get memory preferences
            const calPreferences = await getMemory(store, ["email_assistant", "cal_preferences"], defaultCalPreferences);
            const responsePreferences = await getMemory(store, ["email_assistant", "response_preferences"], defaultResponsePreferences);
            ...
            return {
                messages: [
                    { type: "ai", content: `Error calling model: ${error.message}`, tool_calls: [] }
                ]
            };
        }
    };
 ...
 */
export const createInterruptHandlerNode = (toolsByName, store) => {
    return async (state) => {
...
export const initializeEmailAssistant = async () => {
    ...
    const emailAssistantGraph = new StateGraph(EmailAgentHITLState)
        ...
        .addConditionalEdges("triage_router", (state) => {
        ...
        .addConditionalEdges("triage_interrupt_handler", (state) => {
